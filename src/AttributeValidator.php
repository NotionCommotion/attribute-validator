...
class AttributeValidator implements JsonSerializable
{
    ...
    private function parseFile(string $filename):array
    {
        ...
        $ignoreNamespaceDuplicates=false;
        $skipNext=false;
        $isAbstract = false;
        //$filename = 'src/Kernel.php';
        //$filename = 'src/Processor/HttpMediaProcessor.php';
        $rs = ['namespace'=>null, 'class'=>[], 'trait'=>[], 'interface'=>[], 'abstract'=>[]];
        foreach (\PhpToken::tokenize(file_get_contents($filename)) as $token) {
            if(!$token->isIgnorable()) {
                $name = $token->getTokenName();
                //echo($name.'  '. $token->text.PHP_EOL);
                //echo($name.PHP_EOL);
                switch($name){
                    case 'T_NAMESPACE':
                    ...
                    case 'T_INTERFACE':
                        ...
                        break;
                    case '{':
                        // Temp solution.  TODO check next NAMESPACE declarations.
                        //https://www.php.net/manual/en/language.namespaces.definitionmultiple.php#example-295
                        if($getNextNamespace) {
                            $rs['namespace'] = '_BASE_';
                            $ignoreNamespaceDuplicates = true;
                        }
                        break;
                    case 'T_NAME_QUALIFIED':
                    case 'T_STRING':
                        if($getNextNamespace) {
                            if(array_filter($rs) && !$ignoreNamespaceDuplicates) {
                                throw new AttributeValidatorException(sprintf('Namespace must be defined first in %s', $filename));
                            }
                            else {
